...
inp.args[2] = inp.args[2].toLowerCase();
  ...
  switch(inp.args[0]) {
    ...
    inp.dbConnection.query(`SELECT name FROM customcmds WHERE name=? AND guild="${inp.message.guild.id}"`, inp.args[1], (err1, rows1) => {
      ...
      inp.dbConnection.query(`SELECT * FROM customdelete WHERE`, (err3, rows3) => {
        if (rows1[0]) {
          ...
          if (rows3[0]) {
            ...
            if (newestAction.action === 'recover') return (inp.message.channel.send('The last record says this command is deleted!'))
          }
      };
        //Hurray, is deleted

        inp.dbConnection.query(`INSERT INTO customcmds (type, guild, creator, syntax, value, timestamp, permissions, name, channel, onlyExec) VALUES ("${defaults.type}", "${inp.message.guild.id}", "${inp.message.author.id}","${defaults.syntax}" , "${defaults.value}", "${Math.round(+new Date()/1000).toString()}","${defaults.permissions}" , ${inp.dbConnection.escape(inp.args[1])}, "${inp.message.channel.id}")`, err2 => {
        if (err2) {
          inp.message.channel.send('An error occured!');
...
exports.data = {
  ...
  ignoreSyntax: true,
  timeout: 2000,
  aliases: ['oldcustom_commands', 'oldccmds'],
  blocked: {
    guilds: {},
    users: {}
  },
