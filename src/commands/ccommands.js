...
exports.run = (inp) => {
  ...
  switch(inp.args[0]) {
    ...
    inp.dbConnection.query(`SELECT name FROM customcmds WHERE name=? AND guild=${inp.message.guild.id}`, inp.args[1], (err1, rows1) => {
      ...
      inp.dbConnection.query(`INSERT INTO customcmds (type, guild, creator, syntax, value, timestamp, permissions, name) VALUES (${defaults.type}, ${inp.message.guild.id}, ${inp.message.author.id}, ${defaults.syntax}, ${defaults.value}, ${Math.round(+new Date()/1000).toString()}, ${defaults.permissions}, ${inp.args[1]})`, err2 => {
        if (err2) {
          inp.message.channel.send('An error occured!');
